...
class TournoiRechercheType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        ->add('jeu',ChoiceType::class, [
            'choices' => $this->getJeu(),'autocomplete' => true,
        ])
    ...
    {      
          $query = $this->entityManager->createQuery('SELECT j.nomJeu FROM App\Entity\Jeu j');
           $categories = $query->getArrayResult();
            $choices = [];
    
            foreach ($categories as $category) {
                $choices[$category['nomJeu']] = $category['nomJeu'];
            }
